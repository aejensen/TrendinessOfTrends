/*
  ...
  2) the basis evaluated at x
  3) the basis evaluted at xPred
  4) the basis derivatives evaluated at x
  5) the basis derivaties evaluated at xPred
  as well ad the following hyper-parameters
  6) mean for the prior of mu
  7) shape and scale parameters for the prior of rho
*/
...
functions{
  ...
  matrix pred_rng(real[] xPred, vector y, real[] x, real m, 
    ...
    */
    ...
    matrix[n, nPred] K1_f_1;
    matrix[n, nPred] K1_f_2;
    matrix[n, nPred] K1_f;
    
    matrix[nPred, nPred] K2_f_1;
    matrix[nPred, nPred] K2_f_2;
    ...
    vector[nPred] sim;
    matrix[nPred, 2] result;

    //Get C(x, x) + sigma^2*I
    K_1 = cov_exp_quad(x, alpha[1], rho[1]);
    K_2 = cov_exp_quad(x, alpha[2], rho[2]);
    for(i in 1:n) {
      for(j in 1:n) {
    ...
    */
    //Get C(x, xPred)
    K1_f_1 = cov_exp_quad(x, xPred, alpha[1], rho[1]);
    K1_f_2 = cov_exp_quad(x, xPred, alpha[2], rho[2]);
    for(i in 1:n) {
      for(j in 1:nPred) {
    ...
    //Get C(xPred, xPred)
    K2_f_1 = cov_exp_quad(xPred, alpha[1], rho[1]);
    K2_f_2 = cov_exp_quad(xPred, alpha[2], rho[2]);
    for(i in 1:nPred) {
      for(j in 1:nPred) {
    ...
    */
    ...
    sim = multi_normal_rng(mu_f, C11 + diag_delta);
    
    /* 
    Gather the results
    */
    result[1:nPred, 1] = sim;
    for(i in 1:nPred) {
...
generated quantities {
  ...
  pred = pred_rng(xPred, y, x, mu, w, wPred, alpha, rho, sigma);
}
