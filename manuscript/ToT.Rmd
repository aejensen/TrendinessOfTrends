...
\end{align}
where $\mathcal{F}_t = \left\{\mathbf{Y}, \mathbf{t}\right\}$ consists of the observed outcomes and their sampling times and $\mu_{df}$ and $\Sigma_{df}$ are given in Equation (\ref{eq:jointPost}). 
...
\begin{proposition}
\begin{align}
  \mathrm{ETI}(\mathcal{T} \mid \Theta) = \int_{\mathcal{T}} \frac{\Sigma_{d^2\!f}(t,t)^{1/2}}{\Sigma_{df}(t,t)^{1/2}} \sqrt{1 - \rho(t)^2} \phi\left(\frac{\mu_{df}(t)}{\Sigma_{df}(t,t)^{1/2}}\right)\left[2\phi(\eta(t)) + \eta(t) \Erf\left(\frac{\eta(t)}{\sqrt{2}}\right)\right]\mathrm{d}t
\end{align}
...
\begin{align*}
  \rho(t) = \frac{\Sigma_{df,d^2\!f}(t,t)}{\Sigma_{df}(t,t)^{1/2}\Sigma_{d^2\!f}(t,t)^{1/2}}, \quad \eta(t) = \frac{\mu_{d^2\!f}(t)}{\Sigma_{d^2\!f}(t,t) \sqrt{1-\rho(t)^2}} - \frac{\mu_{df}(t)\rho(t)}{\Sigma_{df}(t,t) \sqrt{1-\rho(t)^2}}
\end{align*}
...
\begin{align*}
  \mu_{f}(\mathbf{t}^\ast) &= ... \\
  \bm{\Sigma}_{f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= ...  
\end{align*}
...
\begin{align*}
  \mu_{df}(\mathbf{t}^\ast) &= ... \\
  \bm{\Sigma}_{df}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= ...
\end{align*}
In order to calculate the Expected Trend Instability index in Proposition \ref{prop:ETIposterior} we further require the posterior moments of $d^2\!f$ and its cross-covariance with $df$. These are given by
\begin{align*}
  \mu_{d^2\!f}(\mathbf{t}^\ast) &= ...\\
  \bm{\Sigma}_{d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= ...\\
  \bm{\Sigma}_{df, d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= ...
\end{align*}
