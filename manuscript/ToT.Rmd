...
The maximum likelihood and Bayesian methods described above differ in the way they approach the hyper-parameters of the model denoted by $\Theta$ in Equation (\ref{eq:generatingProcess}). The maximum likelihood method considers $\Theta$ as fixed parameters in the frequentist sense, while the Bayesian method views these as random variables distributed according to specified priors. Conceptually this gives rise to two different versions of the Trend Direction Index. In the frequentist world using maximum likelihood estimation of $\Theta$, the index is a deterministic function. In the Bayesian model the index is correspondingly random function governed by the posterior distribution of $\Theta$. 

The maximum likelihood estimator is also called the empirical Bayes approach since the prior distributions are estimated from data using the marginal likelihood of the model. This can be seen as an approximation to the Bayesian approach where the hyper-parameters are fixed at their most likely values instead of being integrated out. The advantage of the maximum likelihood method is that it is computationally faster. The advantage of the Bayesian approach is that any uncertainty about the values of the hyper-parameters are directly propagated through the model and into the trend indices.
