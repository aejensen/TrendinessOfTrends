...
We make the dependence of the Trend Direction Index on the model parameters $\Theta$ explicit in the notation in Proposition \ref{prop:TDIposterior} as they enter through the expressions of posterior moments $\mu_{df}$ and $\Sigma_{df}$ in Proposition \ref{prop:GPposterior} and must be estimated from data. From the expression we see that for $\mu_{df}(t + \delta) = 0$, implying that the posterior of $f$ is constant on average at time $t + \delta$, the Trend Direction Index is equal to $\frac{1}{2} = 50\%$.
