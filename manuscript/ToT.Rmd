...
\revision{comes from two recent examples: in the first, the news media in Denmark stated that the trend in the proportion of smokers in Denmark has changed at the end of the year 2018 such that the proportion is now increasing whereas it had been decreasing for the previous 20 years. This statement was based on survey data collected yearly since 1998 and reported by the Danish Health Authority [@sst], and it is critical for the Danish Health Authorities to be able to evaluate and react if there is an actual change in trend. The second example relates to the recent outbreak of COVID-19 in Italy where it is of tremendous importance to determine if the disease spread is increasing or slowing down by considering the trend in number of new cases} (see Figure \ref{fig:rawDataPlot}). 
...
that imposes sparsity on the 1st order differences of the conditional mean. This produces trend estimates that are piece-wise linear with the inherent assumption that changes to the time series are abrupt. The Trend Filtering approach has been further extended to sparsity of $k$th order differences [@ramdas2016trendfiltering] and to a Bayesian framework, where the flexibility of the difference of the conditional mean is controlled through the prior distributions [@kowal2019dynamicshrinkage]
} 
...
\revision{We conclude this section by noting two types of covariance functions that cannot be used for our application. This is because they are in violation with our Assumption A3 which can be verified by a straight-forward calculation. The first case in the M3/2 covariance function in Equation (\ref{eq:covariancefunctions}) while it can be used for estimation of TDI, it cannot be used if also an estimate of ETI is required. This is because it does not fulfill the requirement of mixed third-order partial derivatives of the assumption. Another example is $C_\theta^\text{OU}(s, t) = \alpha^2 \exp\left(-|s-t|/\rho\right)$. This is the covariance function of the Ornstein-Uhlenbeck process which is mean-squared continuous but not mean-squared differentiable. This also fails to satisfied the required assumption.}
...
\end{align*}
where the integral is in practice approximate through the MCMC samples.}
...
\revision{For comparison we employed the Trend Filtering method as implemented in the R package \texttt{genlasso} [@genlasso] on the same simulated data and reported similar measures for its estimated mean, $\widehat{f^\text{TF}}$, and derivative, $\widehat{df^\text{TF}}$ using a 10-fold cross-validation of the penalty parameter. Note that we only compare with the estimates of the latent mean and its derivative since Trend Filtering does not provide a probability distribution for the derivative which is required by the definition of TDI.}
...
\revision{As a final remark on this application we note that the observed data are proportions and therefore not normally distributed. Commonly used transformations for proportions towards normality are the isometric log ratio or the arcsine-square-root or logit functions. In accordance with the comments in Section \ref{sec:gaussianassumptions} we also performed the trend analysis on the logit transformed outcomes. These results are included in the Supplementary Material and did not give rise to different interpretations.}

## Number of new COVID-19 positive cases in Italy {-}

\revision{As a second application we look at the development of the number of new COVID-19 positives in Italy since February 24th 2020. Data was updated each day and made available at the GitHub repository of the Italian Civil Protection Department [@italianCOVID19]. 90 days had passed when this analysis was performed.}
...
\revision{Figure \ref{fig:covid19Fit} shows the result of the analysis. Between day five and six the Trend Direction Index crossed 95\% probability and continued to climb to 100\%. After 29 days (March 24th, 2020) the index proceeded to sharply decrease where it remained for some time. The Italian government imposed quarantine on most of Italy from March 9th, and the sharp drop coincides nicely with the (as of present) expected incubation period of 2--14 days which roughly reflects the incubation and testing period for the individuals who contracted the virus before the March 9th lockdown. After March 24th, the trend was clearly not positive for a long time.}
