...
where
\begin{align*}
  \bm{\mu}(\mathbf{t}^\ast) &= \begin{bmatrix}\mu_\beta(\mathbf{t}^\ast)\\ d\mu_\beta(\mathbf{t}^\ast)\\ d^2\!\mu_\beta(\mathbf{t}^\ast)\end{bmatrix} + \begin{bmatrix}C_\theta(\mathbf{t}^\ast, \mathbf{t})\\ \partial_1 C_\theta(\mathbf{t}^\ast, \mathbf{t})\\ \partial_1^2 C_\theta(\mathbf{t}^\ast, \mathbf{t})\end{bmatrix}\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1} \left(\mathbf{Y} - \mu_\beta(\mathbf{t})\right)\\
  \bm{\Sigma}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= \begin{bmatrix}C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) & \partial_2 C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast)\\ \partial_1 C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) & \partial_1 \partial_2 C_\theta(\mathbf{t}^\ast,\mathbf{t}^\ast)\\ \end{bmatrix} - \begin{bmatrix}C_\theta(\mathbf{t}^\ast, \mathbf{t})\\ \partial_1 C_\theta(\mathbf{t}^\ast, \mathbf{t})\\ \partial_1^2 C_\theta(\mathbf{t}^\ast, \mathbf{t})\end{bmatrix}\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}\begin{bmatrix}C_\theta(\mathbf{t}, \mathbf{t}^\ast)\\ \partial_2 C_\theta(\mathbf{t}, \mathbf{t}^\ast)\\ \partial_2^2 C_\theta(\mathbf{t}, \mathbf{t}^\ast)\end{bmatrix}
\end{align*}
...
Since our model normal. Maximum likelihood estimation is possible through the marginal likelihood of the observed data obtained by integrating out the distribution of the Gaussian Process. The marginal likelihood function is equal to
\begin{align}
...
\end{align}
and the maximum likelihood estimate of the parameters is therefore $\widehat{\Theta^\text{ML}} = \argsup_{\Theta} L(\Theta \mid \mathbf{Y}, \mathbf{t})$. This estimate can then be plugged into the expressions for the Trend Direction Index and the Expected Trend Instability as
\begin{align*}
...
For the Bayesian estimation, the generating model in Equation (\ref{eq:generatingProcess}) must be augmented with a prior distribution of the model parameters
\begin{align*}
  \Theta = (\beta, \theta, \sigma) \sim G(\Psi)
\end{align*}
where $G$ is some family of distribution functions index by a vector of hyper-parameters $\Psi$. We have implemented the Bayesian estimator in Stan [@carpenter2017stan]. Stan is a probabilistic programming language that enables full Bayesian statistical inference using Markov chain Monte Carlo sampling and can be used through R by the \texttt{rstan} package [@rstan]. The Stan programs are available at the first author's Github repository [@gptrendStan].


it is important to account for all the uncertainty in the model. These will then be updated based on the observed data and the Trend Direction Index in Proposition \ref{prop:TDIposterior} is then conditional on these posterior distributions. 
...
@rice1944mathematical showed that the local expected number of zero-crossings of a Gaussian process $X$ at time $t$ is given by
\begin{align*}
\int_{-\infty}^\infty |v|f_{X(t), dX(t)}(0, v)\mathrm{d}v
\end{align*}
where $f_{X(t), dX(t)}$ is the joint density function of $X$ and its derivative $dX$ at time $t$. [\textbf{TODO:} Conditions] By the definition of the local Expected Trend Instability in Equation (\ref{eq:localETI}) we must therefore calculate the Rice formula for the joint posterior distribution of $(df, d^2\!f)$. From Proposition \ref{prop:GPposterior} it follows that the distribution of $(df, d^2\!f) \mid \mathbf{Y}, \mathbf{t}, \Theta$ is bivariate normal for each $t$ where we assume that the $\sigma$-algebra is generated by all observed data. The joint posterior density function evaluated at $(0, v)$ can be factorized as
\begin{align*}
f_{df(t), d^2\!f(t)}(0, v \mid \Theta) = c_1(t) e^{c_2(t)} e^{-c_3(t) v^2 - 2c_4(t) v}
\end{align*}
where $c_1(t), \ldots, c_4(t)$ are functions of time, independent of $v$ and given by
\begin{alignat*}{2}
 c_1(t) &= \frac{1}{2\pi \gamma \sigma (1-\rho^2)^{1/2}} &\quad c_2(t) &= \frac{\mu_{df}(t)^2\gamma^2 -2\mu_{df}(t)\mu_{d^2\!f}(t)\gamma\rho\sigma+\mu_{d^2\!f}(t)^2\sigma^2}{2\gamma^2(\rho^2-1)\sigma^2}\\	
 c_3(t) &= -\frac{1}{2\gamma^2(\rho^2-1)} & c_4(t) &= -\frac{\mu_{df}(t) \gamma \rho - \mu_{d^2\!f}(t) \sigma}{2\gamma^2(\rho^2-1)\sigma}
\end{alignat*}
where  $\mu_{df}$, $\mu_{d^2\!f}$, $\Sigma_{df}$, $\Sigma_{d^2\!f}$ and $\Sigma_{df,d^2\!f}$ are defined in Equations (\ref{eq:appendixPost1}) and (\ref{eq:appendixPost2}). The local Expected Trend Instability can then be written as
\begin{align*}
...
\end{align*}
where the second equality follows by even function. As $c_3(t) > 0$ for $\gamma > 0$ and $-1 < \rho < 1$ by assumption ??? we obtain the following solution for the intergral by using formula 5 in section 3.462 on page 365 of @gradshteyn2014table 
\begin{align*}
\int_{0}^\infty v e^{-c_3(t) v^2 \pm 2c_4(t) v}\mathrm{d}v = \frac{1}{2c_3(t)} \pm \frac{c_4(t)}{2c_3(t)}\sqrt{\frac{\pi}{c_3(t)}}e^{\frac{c_4(t)^2}{c_3(t)}}\left(1 \pm \Erf\left(\frac{c_4(t)}{\sqrt{c_3(t)}}\right)\right)
\end{align*}
where $\Erf\colon\, x \mapsto 2\pi^{-1}\int_0^x e^{-u^2}\mathrm{d}u$ is the error function. This implies that the local Expected Trend Instability is given by
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) &= c_1(t) e^{c_2(t)}\left(\frac{1}{c_3(t)} + \frac{c_4(t)}{c_3(t)}\sqrt{\frac{\pi}{c_3(t)}}e^{\frac{c_4(t)^2}{c_3(t)}}\Erf\left(\frac{c_4(t)}{\sqrt{c_3(t)}}\right)\right)
\end{align*}

Reparametrization
\begin{align}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) &= c_1(t) e^{c_2(t)}\left(\frac{1}{c_3(t)} + \frac{c_4(t)}{c_3(t)}\sqrt{\frac{\pi}{c_3(t)}}e^{\zeta(t)}\Erf\left(\zeta(t)^{1/2}\right)\right)
\end{align}

\begin{align}
  \zeta(t) = \frac{c_4(t)^2}{c_3(t)}
\end{align}
...
#Buuh
Under the conditions of [\textbf{TODO:} ??] we may insert the posterior moments of $(df, d^2\!f) \mid \Theta$ and through a direct integration of the expression in the previous display we obtain that the Expected Trend Instability index generally defined in Equation (\ref{eq:ETIdef}) is given by
\begin{align*}
