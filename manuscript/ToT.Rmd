...
\begin{align*}
\begin{bmatrix}f(\mathbf{t}^\ast)\\ df(\mathbf{t}^\ast)\\ d^2\!f(\mathbf{t}^\ast)\end{bmatrix} \mid \mathbf{Y}, \mathbf{t}, \Theta \sim N\left(\begin{bmatrix}\mu_f(\mathbf{t}^\ast)\\ \mu_{df}(\mathbf{t}^\ast)\\ \mu_{d^2\!f}(\mathbf{t}^\ast)\end{bmatrix}, \begin{bmatrix}\Sigma_{f}(\mathbf{t}^\ast,\mathbf{t}^\ast) & \Sigma_{f,df}(\mathbf{t}^\ast,\mathbf{t}^\ast) & \Sigma_{f,d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast)\\  & \Sigma_{df}(\mathbf{t}^\ast,\mathbf{t}^\ast) & \Sigma_{df,d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast)\\  &  & \Sigma_{d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast) \end{bmatrix}\right)
\end{align*}
where
\begin{align*}
...
The latent Gaussian Process model defined in Equation (\ref{eq:generatingProcess}) requires estimates of the parameter vector $\Theta$ in order to calculate the joint posterior distribution required for the trend indices. Different methods for fitting models with latent Gaussian Processes such as maximum likelihood (also called Empirical Bayes), fully Bayesian estimation and variational Bayes have been considered in the litterature [\textbf{TODO:} ref]. 
...
Bayesian general prior:
\begin{align*}
  \Theta_k \sim \text{Half-T}\left(\widehat{\Theta_k^\text{ML}}, \tau_k^2, \kappa_k\right)
\end{align*}
We have implemented both our model in Stan [@carpenter2017stan]. Stan is a probabilistic programming language that enables full Bayesian statistical inference using Markov chain Monte Carlo sampling and can be used together with R through the \texttt{rstan} package [@rstan]. The Stan programs are available at the first author's Github repository [@gptrendStan].
...
\end{align*}
where $f_{X(t), dX(t)}$ is the joint density function of $X$ and its derivative $dX$ at time $t$ [@rice1944mathematical]. The Expected Trend Instability is defined as the expected number of zero-crossings of $df$ on an interval which implies that the Rice formula must be applied to the joint distribution of $df$ and $d^2\!f$. From Proposition \ref{prop:GPposterior} it follows that the distribution of $(df, d^2\!f) \mid \Theta$ is bivariate normal for each $t$. Its joint density function evaluated at $(0, v)$ can be factorized as
\begin{align*}
...
\end{alignat*}
where  $\mu_{df}$, $\mu_{d^2\!f}$, $\Sigma_{df}$, $\Sigma_{d^2\!f}$ and $\Sigma_{df,d^2\!f}$ are defined in Equations (\ref{eq:appendixPost1}) and (\ref{eq:appendixPost2})

The local Expected Trend Instability can then be written as
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) &= \int_{-\infty}^\infty |v| f_{df(t), d^2\!f(t)}(0, v)\mathrm{d}v \\
