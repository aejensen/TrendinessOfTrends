...
A random function $f$ is a Gaussian Process if and only if the vector $(f(t_1), \ldots, f(t_n))$ has a multivariate normal distribution for every set of evaluation points $(t_1, \ldots, t_n) \subset \mathcal{T}$ with $n < \infty$, and we write $f \sim \mathcal{GP}(\mu(\cdot), C(\cdot, \cdot))$ where $\mu\colon\, \mathcal{T} \mapsto \mathbb{R}$ is the mean function and $C\colon\, \mathcal{T} \times \mathcal{T} \mapsto \mathbb{R}$ is a symmetric, positive definite covariance function [@cramer1967stationary]. We let $\mu(\mathbf{t}) \in \mathbb{R}^p$ and $C(\mathbf{t}, \mathbf{t}) \in \mathbb{R}\times\mathbb{R}$ denote the mean vector and covariance matrix obtained by evaluating $\mu$ and $C$ at a finite vector of time points $\mathbf{t}$. Given observed data of the form $(Y_i, t_i)_{i=1}^n$ we lay out the data generating process by the following hierarchical model 
\begin{align}
...
\end{align}
where $d^k\!\mu_\beta$ is the $k$'th derivative of $\mu_\beta$ and $\partial_j$ denotes the partial derivatives with respect to the $k$'th variable. Let $\mathbf{Y} = (Y_1, \ldots, Y_n)$ be the vector of outcomes observed at times $\mathbf{t} = (t_1,\ldots,t_n) \subset \mathbf{T}$ and $\mathbf{t}^\ast$ any finite dimensional vector of time points not necessarily equal to $\mathbf{t}$. Formulas (\ref{eq:generatingProcess}) and (\ref{eq:latentJoint}) imply that joint distribution of $(f(\mathbf{t}^\ast),df(\mathbf{t}^\ast), \mathbf{Y}) \mid \mathbf{t}, \Theta$ is multivariate normal, and that the marginal distribution of the observed data is $\mathbf{Y} \mid \mathbf{t}, \Theta = N(\mu_\beta(\mathbf{t}), C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I)$ where $I$ is an identity matrix of appropriate dimensions.  Predictions of the latent function and its derivative at $\mathbf{t}^\ast$ are given by the posterior distribution $(f(\mathbf{t}^\ast),df(\mathbf{t}^\ast)) \mid \mathbf{Y}, \mathbf{t}, \Theta$. This distribution is also multivariate normal and its moments are given in Proposition \ref{prop:GPposterior}.

\vspace{0.2cm}

\begin{proposition}
Let the data generating model be defined as in Equation (\ref{eq:generatingProcess}) and $\mathbf{t}^\ast$ any finite vector of time points. The posterior distribution of $(f, df, d^2\!f)$ evaluated at $\mathbf{t}^\ast$ is then given by
\begin{align*}
...
\begin{align*}
  ...
  &= \int_0^\infty N\left(u \mid \mu_{df}(t + \delta), \Sigma_{df}(t + \delta, t + \delta)^{1/2}\right)\mathrm{d}u\nonumber
\end{align*}
where the moments according to Proposition \ref{prop:GPposterior} are given by
\begin{align*}
\mu_{df}(t + \delta) &= d\mu_\beta(t + \delta) + \partial_1 C_\theta(t+\delta, \mathbf{t})\left(C_\theta(\mathbf{t},\mathbf{t}) + \sigma^2 I\right)^{-1} \left(\mathbf{Y} - \mu_\beta(\mathbf{t})\right)\\ 
\Sigma_{df}(t+\delta,t+\delta) &= \partial_1 \partial_2 C_\theta(t+\delta, t+\delta) - \partial_1 C_\theta(t+\delta, \mathbf{t})\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}\partial_2 C_\theta(\mathbf{t}, t+\delta)
\end{align*}
\label{prop:TDIposterior}
\end{proposition}

We make the dependence of the Trend Direction Index on the model parameters $\Theta$ explicit in the notation in Proposition \ref{prop:TDIposterior} as they enter through the expressions of posterior moments $\mu_{df}$ and $\Sigma_{df}$ in Proposition \ref{prop:GPposterior}. The Trend Direction Index is a deterministic function for fixed values of $\Theta$, but in the case where $\Theta$ is estimated from data with its uncertainty taken into account, the index becomes a time-dependent distribution over probabilities.
...
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) = \frac{\Sigma_{d^2\!f}(t,t)^{1/2}}{\Sigma_{df}(t,t)^{1/2}}\left(1-\omega(t)^2\right)^{1/2}\phi\left(\frac{\mu_{df}(t)}{\Sigma_{df}(t,t)^{1/2}}\right)\left(2\phi(\zeta(t)) + \zeta(t)\Erf\left(\frac{\zeta(t)}{2^{1/2}}\right)\right)
\end{align*}
...
\begin{align*}
  \omega(t) = \frac{\Sigma_{df,d^2\!f}(t,t)}{\Sigma_{df}(t,t)^{1/2}\Sigma_{d^2\!f}(t,t)^{1/2}},\quad \zeta(t) = \frac{\mu_{df}(t)\Sigma_{d^2\!f}(t,t)^{1/2}\omega(t)\Sigma_{df}(t,t)^{-1/2} - \mu_{d^2\!f}(t)}{\Sigma_{d^2\!f}(t,t)^{1/2}\left(1 - \omega(t)^2\right)^{1/2}}
\end{align*}
and 
\begin{align*}
  \mathrm{ETI}(\mathcal{I} \mid \Theta) &= \int_\mathcal{I}d\mathrm{ETI}_\mathcal{I}(t \mid \Theta)
...
Maximum likelihood estimation for our model consists of finding the parameters that maximize the marginal likelihood of the observed data and plugging these into the expressions. Since the observation part of the model consists of normal distributed random variables conditional on the latent Gaussian Process, the marginal distribution of $\mathbf{Y}$ is found by integrating out the distribution of the latent Gaussian Process in the conditional distribution $\mathbf{Y} \mid f(\mathbf{t}), \mathbf{t}, \Theta$. This is a multivariate normal distribution with mean $\mu_\beta(\mathbf{t})$ and covariance function $C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I$, and the marginal log likelihood function takes the following form
\begin{align}
...
\end{align}
The maximum likelihood estimate of $\widehat{\Theta^\text{ML}} = \argsup_{\Theta = (\beta, \theta, \sigma)} \log L(\Theta \mid \mathbf{Y}, \mathbf{t})$ can be obtained by numerical optimization or by calculating the roots to the score equations $\nabla_\theta \log L(\Theta \mid \mathbf{Y}, \mathbf{t}) = 0$. The maximum likelihood estimate can then be plugged in to the expressions for the posterior distributions of $(f, df, d^2\!f)$ in Proposition \ref{prop:GPposterior} and simulated at any vector of time point $\mathbf{t}^\ast$. Point estimates of the Trend Direction Index and the Expected Trend Instability are similarly given by
\begin{align*}
  \mathrm{TDI}\left(t, \delta \mid \widehat{\Theta^\text{ML}}\right) \quad \text{and} \quad \mathrm{ETI}\left(\mathcal{I} \mid \widehat{\Theta^\text{ML}}\right)
\end{align*}
where the expressions can be found in Propositions \ref{prop:TDIposterior} and \ref{prop:ETIposterior} respectively.

The maximum likelihood estimator is easy and fast to compute, but it is difficult to propagate the uncertainty of the estimate through to the posterior distributions and the trend indices. This is disadvantageous because in order to do a qualified assessment of trendiness we are not only interested in point estimates but also the their uncertainties. A Baysian estimator, while slower to calculate, provides a straightforward way of including for all uncertainties in the model. For the Bayesian estimator, the generating model in Equation (\ref{eq:generatingProcess}) must be augmented with another level specifying prior distribution of the model parameters
\begin{align*}
  (\beta, \theta, \sigma) \sim G(\Psi)
\end{align*}
where $G$ is some family of distribution functions index by a vector of hyper-parameters $\Psi$. 

The posterior distribution of the trend indices can then be summarized through their $\tau$ quantiles $Q_\tau$ such as
\begin{align*}
  P\left(\mathrm{TDI}\left(t, \delta \mid \Theta\right) \leq \tau\right) = Q_\tau(t, \delta), \quad P\left(\mathrm{ETI}\left(\mathcal{I} \mid \Theta\right) \leq \tau\right) = Q_\tau \quad \tau \in (0; 1)
\end{align*}
which can be estimated from the MCMC samples by their empirical counterparts.

We have implemented the Bayesian estimator in Stan [@carpenter2017stan]. Stan is a probabilistic programming language that enables full Bayesian statistical inference using Markov chain Monte Carlo sampling and can be used through R by the \texttt{rstan} package [@rstan]. The Stan programs are available at the first author's Github repository [@gptrendStan].
...
Different distributions of $Y \mid f$ could be used (e.g. binary outcomes) but then maximum likelihood estimation is not analytically tractable. 
...
\begin{align*}
  ...
  \Sigma_{f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) - C_\theta(\mathbf{t}^\ast, \mathbf{t})\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}C_\theta(\mathbf{t}, \mathbf{t}^\ast)
\end{align*}
...
\begin{align}
  ...
  \Sigma_{df}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= \partial_1 \partial_2 C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) - \partial_1 C_\theta(\mathbf{t}^\ast, \mathbf{t})\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}\partial_2 C_\theta( \mathbf{t}, \mathbf{t}^\ast) \nonumber
\end{align}
...
\begin{align}
  ...
  \Sigma_{d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= \partial_1^2 \partial_2^2 C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) - \partial_1^2 C_\theta(\mathbf{t}^\ast, \mathbf{t})\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}\partial_2^2 C_\theta(\mathbf{t}, \mathbf{t}^\ast) \nonumber \\ 
  \Sigma_{df, d^2\!f}(\mathbf{t}^\ast,\mathbf{t}^\ast) &= \partial_1 \partial_2^2 C_\theta(\mathbf{t}^\ast, \mathbf{t}^\ast) - \partial_1 C_\theta(\mathbf{t}^\ast, \mathbf{t})\left(C_\theta(\mathbf{t}, \mathbf{t}) + \sigma^2 I\right)^{-1}\partial_2^2 C_\theta(\mathbf{t}, \mathbf{t}^\ast) \nonumber
\end{align}
This completes the proof.

## Proof of Proposition \ref{prop:ETIposterior}
@rice1945mathematical showed in section 3.3 that the local expected number of zero-crossings of a Gaussian process $X$ at time $t$ is given by
\begin{align*}
\int_{-\infty}^\infty |v|f_{X(t), dX(t)}(0, v)\mathrm{d}v
\end{align*}
where $f_{X(t), dX(t)}$ is the joint density function of $X$ and its derivative $dX$ at time $t$. [\textbf{TODO:} Conditions] The derive the expression local Expected Trend Instability in Equation (\ref{eq:localETI}) we must apply the Rice formula to the joint posterior distribution of $(df, d^2\!f)$. From Proposition \ref{prop:GPposterior} the distribution of $(df, d^2\!f) \mid \mathbf{Y}, \mathbf{t}, \Theta$ is bivariate normal for each $t$. 

Let $\mu_{df}(t)$, $\mu_{d^2\!f}(t)$, $\Sigma_{df}(s,t)$ and $\Sigma_{d^2\!f}(s,t)$ be defined as in Proposition \ref{prop:GPposterior} and define further
\begin{align*}
  \omega(t) = \frac{\Sigma_{df, d^2\!f}(t,t)}{\Sigma_{df}(t,t)^{1/2}\Sigma_{d^2\!f}(t,t)^{1/2}}
\end{align*}
...
\begin{align*}
f_{df(t), d^2\!f(t)}(0, v \mid \Theta) = c_1(t) e^{c_2(t)} e^{-c_3(t) v^2 - 2c_4(t) v}
\end{align*}
where $c_1, \ldots, c_4$ are functions of time given by
\begin{align*}
 c_1(t) &= \frac{1}{2\pi \Sigma_{df}(t,t)^{1/2}\Sigma_{d^2\!f}(t,t)^{1/2} (1-\omega(t)^2)^{1/2}}\\
 ...
 c_3(t) &= -\frac{1}{2\Sigma_{d^2\!f}(t,t)(\omega(t)^2-1)}\\
 c_4(t) &= -\frac{\mu_{df}(t) \Sigma_{d^2\!f}(t,t)^{1/2} \omega(t) - \mu_{d^2\!f}(t) \Sigma_{df}(t,t)^{1/2}}{2\Sigma_{d^2\!f}(t,t)(\omega(t)^2-1)\Sigma_{df}(t,t)^{1/2}}
\end{align*}
...
\end{align*}
As $c_3(t) > 0$ for $\gamma > 0$ and $|\rho| < 1$ by Assumption [\textbf{TODO}:???] we obtain the following solution for the intergral by using formula 5 in section 3.462 on page 365 of @gradshteyn2014table 
\begin{align*}
\int_{0}^\infty v e^{-c_3(t) v^2 \pm 2c_4(t) v}\mathrm{d}v = \frac{1}{2c_3(t)} \pm \frac{c_4(t)}{2c_3(t)}\frac{\pi^{1/2}}{c_3(t)^{1/2}}e^{\frac{c_4(t)^2}{c_3(t)}}\left(1 \pm \Erf\left(\frac{c_4(t)}{\sqrt{c_3(t)}}\right)\right)
\end{align*}
where $\Erf\colon\, x \mapsto 2\pi^{-1}\int_0^x e^{-u^2}\mathrm{d}u$ is the error function. Combining the equations in the two previous displays we may write the local Expected Trend Instability as
\begin{align*}
...
Defining $\zeta(t) = \sqrt{2}c_4(t)c_3(t)^{-1/2}$ we may rewrite the expression as
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) &= \frac{c_1(t)}{c_3(t)}\left(e^{c_2(t)} + \frac{\pi^{1/2}}{2^{1/2}} e^{\frac{c_4(t)^2}{c_3(t)} + c_2(t)} \zeta(t) \Erf\left(\frac{\zeta(t)}{2^{1/2}}\right)\right)
\end{align*}
and by simple algebra we note that
\begin{align*}
  \frac{c_4(t)^2}{c_3(t)} + c_2(t) = -\frac{\mu_{df}(t)^2}{2\Sigma_{df}(t,t)}, \quad \frac{c_1(t)}{c_3(t)} =  \frac{1}{\pi}\frac{\Sigma_{d^2\!f}(t,t)^{1/2}}{\Sigma_{df}(t,t)^{1/2}}\left(1-\omega(t)^2\right)^{1/2}
\end{align*}
which provides the following result
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) = \frac{1}{\pi}\frac{\Sigma_{d^2\!f}(t,t)^{1/2}}{\Sigma_{df}(t,t)^{1/2}}\left(1-\omega(t)^2\right)^{1/2}\left(e^{c_2(t)} + \pi \phi\left(\frac{\mu_{df}(t)}{\Sigma_{df}(t,t)^{1/2}}\right) \zeta(t) \Erf\left(\frac{\zeta(t)}{2^{1/2}}\right)\right)
\end{align*}
where $\phi\colon\, x \mapsto (2\pi)^{-1/2}e^{-x^2}$ is the density function of the standard normal distribution. Again by algebra we observe that 
\begin{align*}
  c_2(t) = - \frac{1}{2}\left(\zeta(t)^2 + \frac{\mu_{df}(t)^2}{\Sigma_{df}(t,t)}\right)
\end{align*}
so by rewriting $e^{c_2(t)}$ in terms of $\zeta(t)$ and the normal density function and collecting the terms we arrive at the following expression for local Expected Trend Instability
\begin{align*}
d\mathrm{ETI}(t, \mathcal{T} \mid \Theta) = \frac{\Sigma_{d^2\!f}(t,t)^{1/2}}{\Sigma_{df}(t,t)^{1/2}}\left(1-\omega(t)^2\right)^{1/2}\phi\left(\frac{\mu_{df}(t)}{\Sigma_{df}(t,t)^{1/2}}\right)\left(2\phi(\zeta(t)) + \zeta(t)\Erf\left(\frac{\zeta(t)}{2^{1/2}}\right)\right)
\end{align*}
where
\begin{align*}
  \zeta(t) = \frac{\mu_{df}(t)\Sigma_{d^2\!f}(t,t)^{1/2}\omega(t)\Sigma_{df}(t,t)^{-1/2} - \mu_{d^2\!f}(t)}{\Sigma_{d^2\!f}(t,t)^{1/2}\left(1 - \omega(t)^2\right)^{1/2}}
\end{align*}
