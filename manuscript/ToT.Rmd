...
\begin{align*}
\begin{bmatrix}f(\mathbf{t}^\ast)\\ df(\mathbf{t}^\ast)\\ d^2\!f(\mathbf{t}^\ast)\end{bmatrix} \mid \mathbf{Y}, \mathbf{t}, \Theta \sim N\left(\begin{bmatrix}\mu_{f}(\mathbf{t^\ast})\\ \mu_{df}(\mathbf{t^\ast})\\ \mu_{d^2\!f}(\mathbf{t^\ast})\end{bmatrix},   \begin{bmatrix}\Sigma_{f}(\mathbf{t^\ast},\mathbf{t^\ast}) &  \Sigma_{f,df}(\mathbf{t^\ast},\mathbf{t^\ast}) & \Sigma_{f,d^2\!f}(\mathbf{t^\ast},\mathbf{t^\ast})\\  \Sigma_{f,df}(\mathbf{t^\ast},\mathbf{t^\ast})^T & \Sigma_{df}(\mathbf{t^\ast},\mathbf{t^\ast}) & \Sigma_{df,d^2\!f}(\mathbf{t^\ast},\mathbf{t^\ast})\\ \Sigma_{f,d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast})^T & \Sigma_{df,d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast})^T & \Sigma_{d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast})\end{bmatrix}\right)
\end{align*}
where
\begin{align*}
\mu_{f}(\mathbf{t^\ast}) &= \\
\mu_{df}(\mathbf{t^\ast}) &= \\
\mu_{d^2\!f}(\mathbf{t^\ast}) &= \\
\Sigma_{f}(\mathbf{t^\ast}, \mathbf{t^\ast}) &= \\
\Sigma_{df}(\mathbf{t^\ast}, \mathbf{t^\ast}) &= \\
\Sigma_{d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast}) &= \\
\Sigma_{f,df}(\mathbf{t^\ast}, \mathbf{t^\ast}) &= \\
\Sigma_{f,d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast}) &= \\
\Sigma_{df,d^2\!f}(\mathbf{t^\ast}, \mathbf{t^\ast}) &=
\end{align*}
...
The Trend Direction Index was defined generally in Equation (\ref{eq:TCIdef}). Letting the filtration be the sigma algebra of the observed data and their sampling times, we may express the Trend Direction Index through the posterior distribution of $df$. The following proposition states this result.

\vspace{0.2cm}

\begin{proposition}
Let $\mathcal{F}_t$ be the $\sigma$-algebra generated by $\left\{\mathbf{Y}, \mathbf{t}\right\}$. The Trend Direction Index defined in Equation (\ref{eq:TCIdef}) can then be written in terms of the posterior distribution of $df$ as
\begin{align*}
  ...
  &= \frac{1}{2} + \frac{1}{2}\Erf\left(\frac{\mu_{df}(t + \delta)}{2^{1/2}\Sigma_{df}(t + \delta, t + \delta)^{1/2}}\right)
\end{align*}
...
The maximum likelihood estimator is easy to implement and fast to compute, but it is difficult to propagate the uncertainties of the estimated parameter estimate through to the posterior distributions and the trend indices. This is disadvantageous since in order to conduct a qualified assessment of trendiness we are not only interested in point estimates but also the associated uncertainties. A Baysian estimator, while slower to compute, fascilitates a straightforward way to encompass all uncertainties in the final estimates. To specify a Bayesian estimator we must augment the generating model in Equation (\ref{eq:generatingProcess}) with another hierarchical level specifying prior distributions of the model parameters. We therefore add a prior model
\begin{align*}
  \Theta = (\beta, \theta, \sigma) \mid \mathbf{t} \sim G(\Psi)
\end{align*}
to the specification where $G$ is some family of distribution functions indexed by a vector of hyper-parameters $\Psi$. Both $G$ and the value of $\Psi$ must be chosen based from prior knowledge or other subject matter considerations.

\begin{align*}
  P(\mathbf{Y}, f(\mathbf{t}), \Theta \mid \mathbf{t}) &= P(\mathbf{Y} \mid f(\mathbf{t}), \Theta, \mathbf{t})P(f(\mathbf{t}), \Theta \mid \mathbf{t})\\
  &= P(\mathbf{Y} \mid f(\mathbf{t}), \Theta, \mathbf{t})P(f(\mathbf{t}) \mid \Theta, \mathbf{t})P(\Theta)
\end{align*}

\begin{align*}
  P(\mathbf{Y}, \Theta \mid \mathbf{t}) &= P(\Theta) \int P(\mathbf{Y} \mid f(\mathbf{t}), \Theta, \mathbf{t})dP(f(\mathbf{t}) \mid \Theta, \mathbf{t})\\
  &= P(\mathbf{Y} \mid \Theta, \mathbf{t})P(\Theta)
\end{align*}

\begin{align*}
  P(\Theta \mid \mathbf{Y}) = \frac{P(\mathbf{Y} \mid \Theta)dG(\Theta)}{\int P(\mathbf{Y} \mid \Theta)dG(\Theta)}
\end{align*}

Let $\widetilde{\Theta}$ be a random variable distributed according to $P(\Theta \mid \mathbf{Y})$. The posterior distribution of the trend indices can then be summarized through their $\tau$ quantiles $Q_\tau$ such as
\begin{align*}
...
We have implemented both the maximum likelihood and the Bayesian estimator in Stan [@carpenter2017stan] which is a probabilistic programming language enabling full Bayesian inference using Markov chain Monte Carlo sampling and can be used through R by the \texttt{rstan} package [@rstan]. 
...
\end{align*}
As $c_3(t) > 0$ for $\Sigma_{d^2\!f}(t,t) > 0$ and $|\omega(t)| < 1$ for all $t$ by Assumption [\textbf{TODO}:???] we obtain the following solution for the type of integral in the previous display by using formula 5 in section 3.462 on page 365 of @gradshteyn2014table 
\begin{align*}
...
\end{align*}
and by simple arithmetics we note that
\begin{align*}
...
\end{align*}
where $\phi\colon\, x \mapsto (2\pi)^{-1/2}e^{-x^2}$ is the density function of the standard normal distribution. Again by ordinary arithmetics we observe that 
\begin{align*}
