...
functions{
  ...
  matrix gpFit_rng(real[] tPred, real[] t, vector y, 
    ...
    */
    ...
    Gather results
    */
    result[1:p, 1] = sim[1:p]; //f | Y
    for(i in 1:p) {
      result[i, 2] = normal_rng(sim[i], sigma); //Posterior predictive distribution
    }
    result[1:p, 3] = sim[(p+1):(2*p)]; //df | Y
    result[1:p, 4] = sim[(2*p+1):(3*p)]; //ddf | Y
    result[1:p, 5] = calcTDI(p, mu_joint, cov_joint); //TDI
    result[1:p, 6] = calcETI(p, mu_joint, cov_joint); //local ETI
    result[1:p, 7] = mu_joint[1:p]; //E[f | Y]
    result[1:p, 8] = mu_joint[(p+1):(2*p)]; //E[df | Y]
    result[1:p, 9] = mu_joint[(2*p+1):(3*p)]; //E[ddf | Y]
